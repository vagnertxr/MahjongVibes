...
- `server.py` provides a local HTTP server for debugging, phone testing, and service worker checks.
...
- `dist/linux/` from `sh build-linux.sh`.
- `dist/windows/` from `sh build-windows-wine.sh`.
- `dist/windows/Mahjong Vibes.exe` from `build-windows.ps1`.
...
- Python envelope: run `server.py` for local debug and LAN testing.
- PyInstaller envelope: package `launcher.py` with the static assets.
